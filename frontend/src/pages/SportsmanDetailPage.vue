      ...
      <template v-else-if="profile">
        ...
        <div class="w-full md:w-60 shrink-0 flex flex-col items-center pt-6 md:pt-10 px-4 md:px-6 pb-4 md:pb-6">
          <div class="flex flex-col items-center gap-4 flex-1">
            ...
            <div class="relative group cursor-pointer" @click="avatarInput?.click()">
              <div class="w-32 h-32 md:w-48 md:h-48 rounded-full overflow-hidden shrink-0 bg-blue-500 flex items-center justify-center">
            ...
            <div class="text-center">
              ...
              <div class="text-xs text-neutral-500 mt-1">Спортсмен</div>
            </div>
          </div>
          <button
          ...
          <div v-if="showLogoutConfirm" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div class="bg-white rounded-2xl shadow-xl p-6 w-80 flex flex-col gap-4">
              ...
              <div class="flex gap-3 mt-1">
                ...
                <button
                  @click="auth.logout()"
                  class="flex-1 py-2 rounded-xl bg-red-500 hover:bg-red-600 transition-colors text-sm font-semibold text-white"
                >Выйти</button>
              </div>
            </div>
          </div>
        ...
        <div class="flex-1 md:overflow-y-auto px-4 md:px-8 py-4 md:py-8 flex flex-col gap-0">
          ...
          <div class="space-y-3">
            ...
            <InfoRow label="Возраст" :value="profile.age ? `${profile.age} лет` : '—'" />
            <InfoRow label="Пол" :value="genderLabel(profile.gender)" />
            <InfoRow label="Позиция" :value="positionLabel[profile.position] ?? profile.position ?? '—'" />
            <InfoRow label="Специализация" :value="specializationLabel(profile.specialization)" />
            <InfoRow label="Рост" :value="profile.height ? `${profile.height} см` : '—'" readonly />
            <InfoRow label="Вес" :value="profile.weight ? `${profile.weight} кг` : '—'" readonly />
            <InfoRow v-if="team" label="Текущая команда" :value="team" />
            <InfoRow v-if="groups.length" label="Группы" :value="groups.map((g: any) => g.name).join(', ')" />
          </div>

          <div class="border-t border-neutral-900 my-6" />

          <!-- Контакты + смена пароля -->
          <div class="space-y-3">
...
function genderLabel(g: string): string {
  if (g === 'M' || g === 'm') return 'Мужской'
  if (g === 'F' || g === 'f') return 'Женский'
  return '—'
}

function specializationLabel(s: string): string {
  if (s === 'Football') return 'Футбол'
  if (s === 'Minifootball') return 'Мини-футбол'
  return s ?? '—'
}
...
async function load() {
  loading.value = true
  try {
    const [sm, user] = await Promise.allSettled([
      api.get('/sportsman/me'),
      api.get('/users/me'),
    ])
    ...
    if (profile.value?.id) {
      const [gr, tm] = await Promise.allSettled([
        api.get(`/group/sportsman/${profile.value.id}`),
        profile.value.teamId ? api.get(`/team/${profile.value.teamId}`) : Promise.resolve(null),
      ])
      groups.value = (gr as any).value?.data?.data ?? []
      team.value = (tm as any).value?.data?.data?.name ?? ''
    }
  } finally {
    loading.value = false
  }
