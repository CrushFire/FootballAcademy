...
onMounted(async () => {
  const athleteId = route.params.id ? Number(route.params.id) : null

  const [cRes, mRes, grpRes] = await Promise.allSettled([
    api.get('/schedule'),
    api.get('/match'),
    athleteId ? api.get(`/group/sportsman/${athleteId}`) : Promise.resolve(null),
  ])
  if (cRes.status === 'fulfilled') {
    let allClasses: any[] = cRes.value.data.data ?? []
    if (athleteId && grpRes.status === 'fulfilled' && grpRes.value) {
      const groups: any[] = grpRes.value.data.data ?? grpRes.value.data ?? []
      const groupIds = groups.map((g: any) => g.id ?? g.Id)
      if (groupIds.length) allClasses = allClasses.filter(c => groupIds.includes(c.groupId ?? c.GroupId))
    }
    classes.value = allClasses
  }
  if (mRes.status === 'fulfilled') matches.value = (mRes.value.data.data ?? []).filter((m: any) => m.status === 'Scheduled')
  loading.value = false
})
</script>
