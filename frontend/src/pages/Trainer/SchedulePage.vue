<template>
  <div class="p-3 h-full">
    <TrainerPageCard color="blue" class="h-full flex flex-col overflow-y-auto">
      ...
      <div v-else>
        <!-- ДЕСКТОП: классическая 7-колоночная сетка -->
            ...
            >
              ...
              <template v-for="ev in eventsForDay(day.iso)" :key="'me' + ev.key">
                ...
                <div v-else class="rounded-md px-1.5 py-1 bg-blue-50 border border-blue-100">
                  ...
                  <div class="text-[10px] text-blue-400 truncate">{{ ev.groupName }}</div>
                </div>
              </template>
            </div>
          </div>
...
// Для мобильной 5-колоночной сетки: только дни текущего месяца, без хвостов.
