<template>
  <div class="bg-white rounded-2xl border border-neutral-200 shadow-sm">
    ...
    <div class="px-3 md:px-5 py-3 md:py-4 flex items-center justify-between gap-2 md:gap-4">
      <!-- Кнопка назад + инфо -->
      <div class="flex items-center gap-2 md:gap-4 flex-1 min-w-0">
        <button @click="$emit('back')"
          ...
          <span class="hidden md:inline">К списку</span>
        </button>

        <!-- Аватарка спортсмена: на всю высоту плашки. -->
        <img v-if="avatarUrl" :src="avatarUrl" class="w-12 h-12 md:w-16 md:h-16 rounded-full object-cover object-top border-2 border-blue-200 shrink-0" />
        <div v-else class="w-12 h-12 md:w-16 md:h-16 rounded-full bg-blue-100 flex items-center justify-center shrink-0 border-2 border-blue-200">
          <span class="text-blue-600 font-bold text-base md:text-xl">{{ initials }}</span>
        </div>
      ...
      <button @click="$emit('edit')"
        class="flex items-center gap-1.5 px-2 md:px-3 py-1.5 rounded-xl border border-green-200 bg-green-50 text-green-600 text-xs font-semibold hover:bg-green-100 transition-colors shrink-0">
